...
class BackSideCircle: CAShapeLayer, ShapeLayerProtocol {
    required init(size: CGSize, fillColor: CGColor) {
        ...
        for _ in 1...15 {
            let randomXCenter = Int.random(in: 0...Int(size.width))
            let randomYCenter = Int.random(in: 0...Int(size.height))
            let center = CGPoint(x: randomXCenter, y: randomYCenter)
            
            let randomRadius = Int.random(in: 5...15)
